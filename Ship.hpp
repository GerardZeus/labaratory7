...
namespace mt
{
	...
	public:
		...
		bool Setup(float x, float y) // Фунция для установки начальных параметров корабля
		{
			...
			m_spriteShip.setOrigin(m_textureShip.getSize().x / 2, m_textureShip.getSize().y / 2); // Устанавливаем центр корабля в центральной его точке(изничально в левом углу спрайта)
